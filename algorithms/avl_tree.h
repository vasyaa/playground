...
public:
    ...
    avl_tree_iterator(T *p) : current(p) {}

    avl_tree_iterator(const avl_tree_iterator &it) : current(it.current) {}
    ...
    bool operator==(const avl_tree_iterator &o) { return current == o.current; }

    bool operator!=(const avl_tree_iterator &o) { return !operator==(o); }

    avl_tree_iterator operator++() {
        throw std::runtime_error("operator++ not implemented");
    }

    node_type *current_ptr() { return current; }
...
public:
    ...
    typedef internal::avl_tree_iterator<node_type, value_type &, value_type *>
        iterator;
...
private:
    ...
    uint64_t height(node_type *p) { return p ? p->height : 0; }

    int bfactor(node_type *p) { return height(p->right) - height(p->left); }
    ...
    // remove
    node_type *findmin(node_type *p) { return p->left ? findmin(p->left) : p; }
