...
namespace internal {
    struct Interval {
        ...
        Interval() = default;
        Interval(char id_, int deadline_, int profit_): id(id_), deadline(deadline_), profit(profit_) {}
    };
...
public:
    ...
    void init(const data_type& data_){
        data = data_;
    }
...
void test() {
    SchedulingJobsSolver::data_type data = { {'a', 2, 100}, {'b', 1, 19}, {'c', 2, 27},
            {'d', 1, 25}, {'e', 3, 15}};
