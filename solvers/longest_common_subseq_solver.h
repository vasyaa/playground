...
public:
    ...
    int lcs_reqursive_back(data_type &a, data_type &b, size_t m, size_t n) {
        ...
        if (a[m - 1] == b[n - 1])
            ...
            return std::max(lcs_reqursive_back(a, b, m, n - 1),
                            lcs_reqursive_back(a, b, m - 1, n));
    }

    int solve() { return lcs_reqursive_back(a, b, a.size(), b.size()); }
};
...
class LongestCommonSubseqSolver<RECURSIVE_SOLVER>
    : public LongestIncreasingSubseqSolverBase {
public:
    int solve() { return lcs_reqursive(a, b, 0, 0); }
...
private:
    int lcs_reqursive(data_type &a, data_type &b, size_t m, size_t n) {
        ...
        if (a[m] == b[n])
            ...
            return std::max(lcs_reqursive(a, b, m, n + 1),
                            lcs_reqursive(a, b, m + 1, n));
    }
};

template <>
class LongestCommonSubseqSolver<MEMOIZATION_SOLVER>
    : public LongestIncreasingSubseqSolverBase, public DPBase {
public:
    void init(const data_type &a_, const data_type &b_) {
...
private:
    int lcs_memoization(data_type &a, data_type &b, size_t m, size_t n) {
        ...
        if (a[m] == 0 || b[n] == 0)
            ...
            dp[m][n] = std::max(lcs_memoization(a, b, m, n + 1),
                                lcs_memoization(a, b, m + 1, n));
...
public:
    int solve() { return lcs_dp(a, b, a.size(), b.size()); }
...
inline void test() {
    {
        ...
        int rc = s.solve();
        assert(rc == 4);
        std::cout << "longest_common_subseq_solver solve_reqursive_back OK"
                  << std::endl;
    }

    {
        ...
        int rc = s.solve();
        assert(rc == 4);
        std::cout << "longest_common_subseq_solver solve_reqursive OK"
                  << std::endl;
    }

    {
        ...
        int rc = s.solve();
        assert(rc == 4);
        std::cout << "longest_common_subseq_solver solve_memoization OK"
                  << std::endl;
    }
